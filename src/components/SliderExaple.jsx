      ...
      >
        {slides.map((slide, index) => (
          <div key={index} className="min-w-full flex-shrink-0">
            <img src={slide} alt={`Slide ${index + 1}`} className="w-full h-auto rounded-lg shadow-md" />
          </div>
        ))}
      </div>
      <div className="absolute inset-0 flex items-center justify-between p-4">
        <button onClick={handlePrev} className="bg-gray-800 text-white p-2 rounded-full shadow-md">
          <FaArrowLeft />
        </button>
        <button onClick={handleNext} className="bg-gray-800 text-white p-2 rounded-full shadow-md">
          <FaArrowRight />
        </button>
      </div>
    </div>
